...
This project uses K-Means to identify underlying physiological sub-groups of patients based on health metrics without using target labels.

- **Dataset**: `diabetes.csv` (Pima Indians Diabetes Dataset)
- **Validation outcome**: `Outcome` (0: Non-Diabetic, 1: Diabetic - used only for post-clustering verification)
- **Features Used**:
...
main.py
  ...
  |
  +-- kmeans_clustering.py   (KMeansClusteringService - train, evaluate,
...
| `target_column`      | `DataConfig`  | `'Outcome'`  | Outcome label column for cluster congruence table|
