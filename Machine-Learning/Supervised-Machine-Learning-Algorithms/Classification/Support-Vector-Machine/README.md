...
This project implements SVM to classify cell nuclei descriptions as **Malignant** or **Benign** based on physical characteristics extracted from digitized images of fine needle aspirates (FNA) of breast masses.

**Dataset:** `breast_cancer_wisconsin.csv`

**Target Variable:** `diagnosis`
Possible classes: `M` (Malignant), `B` (Benign)

**Features Used:**
30 physical attributes characterizing cell nuclei, including:
- `mean radius` (mean of distances from center to points on the perimeter)
- `mean texture` (standard deviation of gray-scale values)
- `mean perimeter`
- `mean area`
- `mean smoothness` (local variation in radius lengths)
- `mean compactness` ($\text{perimeter}^2 / \text{area} - 1.0$)
...
- SVM is highly effective in high-dimensional spaces (such as this 30-feature dataset).
- It is robust to overfitting when the number of features is relatively large compared to the sample size.
- Standardizing the features allows the radial basis function (RBF) kernel to calculate optimal decision boundary hyperplanes.
...
| `target_column`      | `DataConfig`  | `'diagnosis'`| Target label column in dataset                   |
