# ============================================================================
# Data Loader Module for SVM Classification Pipeline
# ============================================================================
# Handles all data I/O, validation, exploration, scaling, and preprocessing.
...
class DataLoaderService:
    """Service responsible for loading, validating, preprocessing, and scaling the dataset.

    Responsibilities:
        1. Read the CSV from disk and validate its schema.
        2. Log an exploratory summary (shape, types, nulls, class distribution).
        3. Standardize features to have zero mean and unit variance (critical for SVM).
        4. Encode categorical target labels to integers.
        5. Split into train/test sets with optional stratification.

    Design decision -- Why StandardScaler is critical for SVM?
        Unlike Decision Trees which are scale-invariant, SVM finds the hyperplane
        that maximizes the margin. This optimization problem relies on Euclidean
        distances between data points. If features have different scales, features
        with larger magnitudes will dominate the distance calculation, distorting the
        hyperplane. Standardizing ensures each feature contributes equally.

    Design decision -- Why fit the scaler ONLY on training data?
        Fitting the scaler on the entire dataset before splitting causes information
        leakage (the mean and variance of the test set are leaked to the training set).
        To prevent leakage, the scaler is fit only on the training set and applied to
    ...
    def load_and_prepare(
        ...
        Returns:
            A tuple of (X_train_scaled, X_test_scaled, y_train, y_test) as numpy arrays.
    ...
    def _preprocess(
        ...
        if self._data_config.drop_columns:
            self._logger.info("Dropped columns: %s", self._data_config.drop_columns)
    ...
    def _scale_features(
        ...
        """Apply StandardScaler to training features and transform test features.
        ...
        """
        self._logger.info("Fitting StandardScaler on training features and scaling...")
        x_train_scaled = self._scaler.fit_transform(x_train)
        x_test_scaled = self._scaler.transform(x_test)
        
        self._logger.info("Scaling complete. Feature distributions scaled to mean=0, std=1.")
