...
This project predicts whether an online financial transaction is fraudulent based on transaction attributes, account balances, and transaction types.

**Dataset:** `onlinefraud.csv`

**Target Variable:** `isFraud`
Possible classes: `0` (Genuine), `1` (Fraudulent)
...
| `oldbalanceOrg`  | Initial balance before the transaction for the origin customer |
| `newbalanceOrig` | New balance after the transaction for the origin customer     |
| `oldbalanceDest` | Initial balance before the transaction for the destination customer |
| `newbalanceDest` | New balance after the transaction for the destination customer |

**Columns excluded from features:**

- `nameOrig` - origin account identifier (high cardinality).
- `nameDest` - destination account identifier (high cardinality).
- `isFlaggedFraud` - rule-based system flag (causes data leakage).

**Why Decision Tree is a good fit here:**
- The resulting tree produces explicit if-then rules (e.g., "If amount > 200,000 AND type is TRANSFER AND newbalanceOrig == 0, predict Fraud") that are directly interpretable and deployable in rule engines.
- Decision Trees handle mixed data types (numerical and categorical features) naturally.
- They are computationally efficient for high-volume real-time transaction screening.
...
| `target_column`      | `DataConfig`  | `'isFraud'`  | Column being predicted              |
