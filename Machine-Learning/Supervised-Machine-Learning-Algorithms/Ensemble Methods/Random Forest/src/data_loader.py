...
class DataLoaderService:
    ...
    Responsibilities:
        ...
        3. Encode categorical features and target labels using LabelEncoder.
        4. Split into train/test sets with optional stratification.

    Design decision -- Why no feature scaling is needed for Random Forest?
        Random Forest is an ensemble of Decision Trees. Individual trees make splits
        based on threshold inequalities (e.g. feature_x <= threshold). These splits
        depend purely on the rank order of the features, not their absolute scale.
        Therefore, scaling is mathematically a no-op for Random Forest.

    Design decision -- Why encode categorical columns?
        scikit-learn's `RandomForestClassifier` implementation expects all input
        features to be numeric. We encode categories into ordinal integers using
        LabelEncoder.
    """
    ...
    def load_and_prepare(
        ...
        # Apply scaling if configured (disabled by default)
