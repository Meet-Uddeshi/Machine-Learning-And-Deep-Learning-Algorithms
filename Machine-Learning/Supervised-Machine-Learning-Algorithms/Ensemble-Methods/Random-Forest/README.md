...
### Indian Railway Maintenance Prediction

This project uses Random Forest to predict whether a train requires **Maintenance** based on operational metrics, environmental factors, and inspection scores.

**Dataset:** `indian_railway_failure_detection_maintenance_v2.csv`

**Target Variable:** `maintenance_required`
Possible classes: `0` (No Maintenance Needed), `1` (Maintenance Required)

**Features Used:**
Operational, sensor, and environmental metrics, including:
- `region` - geographical region (categorical)
- `season` - season of operation (categorical)
- `train_type` - type of train (categorical)
- `train_age_years` - age of the train in years
- `average_speed_kmph` - average train speed
- `distance_travelled_km` - total distance travelled
- `ambient_temperature_c` - ambient temperature during operation
- `humidity_percent` - humidity level
- `rainfall_mm` - rainfall level
- `wheel_wear_percent` - percentage of wheel wear
- `track_vibration_level` - measured track vibration level
- `rail_wear_mm` - rail wear in mm
- `bearing_temperature_c` - bearing temperature in Celsius
- `axle_temperature_c` - axle temperature in Celsius
- `brake_pad_wear_percent` - percentage of brake pad wear
- `brake_pressure_psi` - brake pressure in psi
- `battery_voltage` - battery voltage reading
- `last_maintenance_days` - number of days since last maintenance
- `sensor_health_index` - health index of onboard sensors
- `inspection_score` - score from physical inspections
- `delay_minutes` - recorded delay minutes

**Columns excluded from features:**
- `train_id` - unique identifier (non-predictive).
- `failure_type` - target-linked categorical feature (causes data leakage).
- `failure_severity` - target-linked categorical feature (causes data leakage).
- `risk_score` - calculated risk score (causes data leakage).
...
| `target_column`      | `DataConfig`  | `'maintenance_required'` | Target label column in dataset           |
